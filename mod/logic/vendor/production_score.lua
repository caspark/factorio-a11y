    ...
    for name, recipes in pairs(nil_prices) do
        if #recipes > 0 then
            ...
            for k, recipe in pairs(recipes) do
                    ...
                    if ingredient.name == name then
                        ...
                        if ingredient_price then
                            ingredient_value = ingredient_value
                                                   + (ingredient_price * ingredient.amount)
                        else
                ...
                local reverse_price = (product_value - energy_addition(recipe, product_value))
                                          / ingredient_multiplier(recipe.ingredients, param) -- Not perfect, but close enough
    ...
    get_price_recursive = function(name, current_loop, loop_force)
        ...
        for k, recipe in pairs(entry) do
            ...
            if this_recipe_cost > 0 then
                this_recipe_cost = (this_recipe_cost * ingredient_multiplier(recipe, param))
                                       + energy_addition(recipe, this_recipe_cost)
                if recipe_cost then
